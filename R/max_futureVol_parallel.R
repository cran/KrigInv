max_futureVol_parallel <- function(lower, upper, optimcontrol=NULL,
                                   batchsize,
                                   integration.param,
                                   T, model, new.noise.var=0,typeEx=">"){

  optim.option <- optimcontrol$optim.option
  if(is.null(optim.option)) optim.option <- 2
  if(is.null(typeEx)) typeEx <- ">"
  ...
  if(typeEx==">"){
    ...
    pn <- pnorm((T-intpoints.oldmean)/intpoints.oldsd)
  }

  # When running EGIparallel this is never NULL
  # calling max_futureVol_parallel with alpha=NULL triggers
  # conservativeEstimate calculation
  if(is.null(alpha)){
    consLevel <- 0.95; n_discrete_design<-500*model@d
    CE_design<-as.matrix (sobol (n = n_discrete_design, dim = model@d))
    colnames(CE_design) <- colnames(model@X)
    current.pred <- predict.km(object = model, newdata = CE_design, type = "UK",cov.compute = TRUE)
    current.pred$cov <- current.pred$cov +1e-7*diag(nrow = nrow(current.pred$cov),ncol = ncol(current.pred$cov))

    current.CE <- conservativeEstimate(alpha = consLevel, pred=current.pred,design=CE_design, threshold=T, pn = NULL,type = typeEx, verb = 1, lightReturn = TRUE, algo = "GANMC")
    alpha <- current.CE$lvs
  }

  pn_bigger_than_alpha <- (pn>alpha)+0
  pn_lower_than_alpha <- 1-pn_bigger_than_alpha

  if(is.null(integration.weights)) current.vorob <- 0 #mean(pn)
  ...
  fun.optim <- vorobVol_optim_parallel
  ########################################################################################
  ...
  if(optimcontrol$method=="discrete"){
    ...
    if(ncol(optim.points)==d){
      ...
      fun.optim <- vorobVol_optim_parallel2
      colnames(optim.points) <- colnames(model@X)
      all.crit <- matrix(1,nrow = nrow(optim.points),ncol = batchsize)
      ...
      for (j in 1:batchsize){
        ...
        for (i in 1:nrow(optim.points)){
          all.crit[i,j] <- fun.optim(x=t(optim.points[i,]), integration.points=integration.points,integration.weights=integration.weights,
                                     intpoints.oldmean=intpoints.oldmean,intpoints.oldsd=intpoints.oldsd,
                                     precalc.data=precalc.data,T=T, model=model, new.noise.var=new.noise.var,
                                     other.points=other.points,batchsize=j,alpha=alpha,current.crit=current.vorob,typeEx=typeEx)
        }
        ibest <- which.max(all.crit[,j])
      ...
      o$value <- max(all.crit[,j]); o$value <- as.matrix(o$value); colnames(o$value) <- colnames(model@y)
      ...
      for (i in 1:nrow(optim.points)){
        all.crit[i] <- fun.optim(x=t(optim.points[i,]), integration.points=integration.points,integration.weights=integration.weights,
                                 intpoints.oldmean=intpoints.oldmean,intpoints.oldsd=intpoints.oldsd,
                                 precalc.data=precalc.data,T=T, model=model, new.noise.var=new.noise.var,
                                 batchsize=batchsize,alpha=alpha,current.crit=current.vorob,typeEx=typeEx)
      }
  ...
  if(optimcontrol$method=="genoud"){
    ...
    if(optim.option==1){
      ...
      o <- genoud(fn=fun.optim, nvars=d*batchsize, max=TRUE, pop.size=optimcontrol$pop.size,
                  ...
                  current.crit=current.vorob,typeEx=typeEx)
      ...
      for (i in 1:batchsize){
        ...
        o <- genoud(fn=fun.optim, nvars=d, max=TRUE, pop.size=optimcontrol$pop.size,
                    max.generations=optimcontrol$max.generations,wait.generations=optimcontrol$wait.generations,
                    hard.generation.limit=TRUE, starting.values=optimcontrol$parinit, MemoryMatrix=TRUE,
                    Domains=domaine, default.domains=10, solution.tolerance=0.000000001,
                    boundary.enforcement=2, lexical=FALSE, gradient.check=FALSE, BFGS=TRUE,
                    ...
                    intpoints.oldmean=intpoints.oldmean,intpoints.oldsd=intpoints.oldsd,
                    precalc.data=precalc.data,integration.weights=integration.weights,
                    new.noise.var=new.noise.var,batchsize=i,current.crit=current.vorob,
                    alpha=alpha,typeEx=typeEx)
    ...
    return(list(par=o$par, value=o$value, alpha=alpha))
  }
}
